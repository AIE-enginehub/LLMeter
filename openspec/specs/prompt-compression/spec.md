# Capability — prompt-compression

Compress natural-language prose in proxied request bodies before forwarding upstream,
reducing input tokens sent to the external LLM while preserving the client-facing API
contract. Implemented in `src/compress.rs` + `src/proxy.rs`; controlled via the global
`compression` setting, per-model `compression_enabled`, and the `X-LLMeter-Compress` header.
